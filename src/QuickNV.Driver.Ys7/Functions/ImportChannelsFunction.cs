...
namespace QuickNV.Driver.Ys7.Functions
{
    public class ImportChannelsFunction
    {
        ...
        public static ChannelInfo ApiChannelInfo2ChannelInfo(string deviceId, QuickNV.YS7.Model.ChannelInfo t)
        {
            return new ChannelInfo()
            {
                DeviceId = deviceId,
                Id = t.channelNo.ToString(),
                Name = t.channelName,
                DriverConfig = JsonSerializer.Serialize(new ChannelConfig()
                {
                    ChannelId = t.channelNo
                })
            };
        }

        internal static QuickNV.YS7.Model.ChannelInfo[] GetDeviceChannels(string deviceId)
        {
            var ret = Agent.Instance.Ys7Context.Ys7Client.GetDeviceChannels(deviceId).Result;
            if (!ret.IsSuccess)
        ...
        private static List<FieldForGet> innerGet(DeviceContext deviceContext, FunctionRequest functionRequest)
        {
            ...
            foreach (var channel in GetDeviceChannels(deviceContext.Model.Config.Ys7DeviceSerial))
            {
                if (alreadyImportChannelDict.ContainsKey(channel.channelNo.ToString()))
                    continue;
        ...
        public static Response Invoke(QpChannel channel, Request request)
        {
            ...
            if (request.FieldIds != null)
            {
                ...
                {
                    var deviceChannels = GetDeviceChannels(deviceContext.Model.Config.Ys7DeviceSerial);
                    var channelsField = functionRequest.Fields.FirstOrDefault(t => t.Id == VAR_CHANNELS);
                    ...
                    foreach (var channelField in channelsField.Children)
                    {
                        ...
                        var channelModel = deviceChannels.First(t => t.channelNo.ToString() == channelId);
                        if (channel == null)
                            continue;

...
namespace QuickNV.Driver.HikvisionDeviceNetwork.Functions
{
    public class ImportDevicesFunction
    {
        ...
        private const string STEP_1_VAR_ENCODING = nameof(STEP_1_VAR_ENCODING);
        ...
        private static List<FieldForGet> step_1(FunctionRequest functionRequest)
        {
            ...
            list.Add(new FieldForGet()
            {
                ...
                Value = functionRequest == null ? "8000" : CorrectPort(functionRequest.GetFieldValue(tmpKey))
            });
            ...
            tmpKey = nameof(STEP_1_VAR_ENCODING);
            list.Add(new FieldForGet()
            {
                Id = tmpKey,
                Name = "字符编码",
                Type = FieldType.InputSelect,
                Value = functionRequest == null ? "GB18030" : functionRequest.GetFieldValue(tmpKey),
                Input_AllowBlank = false,
        ...
        private static List<FieldForGet> step_2(FunctionRequest functionRequest)
        {
            ...
            var encoding = functionRequest.GetFieldValue(STEP_1_VAR_ENCODING);
            var list = new List<FieldForGet>
            {
                new FieldForGet()
                {
            ...
            using (var session = HvSession.Login(host, port, username, password, Encoding.GetEncoding(encoding)))
            {
                ...
                else if (functionRequest.IsFieldIdsMatch(STEP_2_BUTTON_IMPORT))
                {
                    ...
                    using (var session = HvSession.Login(host, port, username, password, Encoding.GetEncoding(encoding)))
                    {
                        ...
                        deviceInfo = new DeviceInfo()
                        {
                            ...
                            Model = deviceConfig.TypeName,
                            SerialNumber = deviceConfig.Serial,

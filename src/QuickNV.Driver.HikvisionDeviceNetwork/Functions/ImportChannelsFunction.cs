...
namespace QuickNV.Driver.HikvisionDeviceNetwork.Functions
{
    public class ImportChannelsFunction
    {
        ...
        private static List<FieldForGet> innerGet(DeviceContext deviceContext, FunctionRequest functionRequest)
        {
            ...
            foreach (var channel in deviceContext.HvSession.ChannelService.AllChannels)
            {
            ...
            list.Add(new FieldForGet()
            {
                ...
                Type = FieldType.InputSelect,
                PostOnChanged = true,
                Value = functionRequest == null ? HvStreamType.Sub.ToString() : functionRequest.GetFieldValue(tmpKey),
                Input_AllowBlank = false,
                InputSelect_Options = new Dictionary<string, string>()
                {
                    [HvStreamType.Main.ToString()] = "主码流",
                    [HvStreamType.Sub.ToString()] = "子码流",
                    [HvStreamType.StreamType3.ToString()] = "第三码流",
                    [HvStreamType.StreamType4.ToString()] = "第四码流"
                }
            });
        ...
        public static Response Invoke(QpChannel channel, Request request)
        {
            ...
            if (functionRequest != null)
            {
                if (functionRequest.IsFieldIdsMatch(BUTTON_IMPORT))
                {
                    ...
                    {
                        StreamType = Enum.Parse<HvStreamType>(functionRequest.GetFieldValue(VAR_STREAM_TYPE)),

...
namespace QuickNV.Driver.GB28181.Functions
{
    public class ImportChannelsFunction
    {
        ...
        private static List<FieldForGet> innerGet(DeviceContext deviceContext, FunctionRequest functionRequest)
        {
            ...
            foreach (var channelContext in deviceContext.GetChannels())
            {
                if (alreadyImportChannelDict.ContainsKey(channelContext.Model.Id))
                    continue;
                waitImportChannelList.Add(new DriverChannel<ChannelConfig>(channelContext.Model));
            }
        ...
        {
            var deviceContext = Agent.Instance.SipServer.GetDevice(request.DeviceId);
                ...
                else if (functionRequest.IsFieldIdsMatch(BTN_IMPORT_SELECTED))
                {
                    ...
                    foreach (var channelField in channelsField.Children)
                    {
                        ...
                        if (channelContext == null)
                            continue;
                        channelsList.Add(channelContext.Model);
                    }
